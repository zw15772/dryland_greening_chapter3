...
class data_processing():
    ...
    def extend_GS(self):
        ...
        f= result_root + rf'\extract_GS\OBS_LAI\carbontracker.npy'
        outf=result_root + rf'extract_GS\\OBS_LAI_extend\carbontracker.npy'
...
class statistic_analysis():
    ...
    def trend_analysis(self):
        ...
        fdir = result_root +'extract_GS\OBS_LAI_extend\\'
        outdir = result_root + rf'trend_analysis\\relative_change\\OBS_LAI_extend\\'
        Tools().mk_dir(outdir, force=True)
        for f in os.listdir(fdir):
            if not 'noy' in f:
                continue
...
class moving_window():
    ...
    def moving_window_CV_trends(self):
        ...
        f = result_root + rf'extract_window\extract_detrend_original_window_CV\\GPCC.npy'
        outdir = result_root + rf'\\extract_window\\extract_detrend_original_window_CV\\'
        T.mk_dir(outdir, force=True)
        dic=T.load_npy(f)
        result_dic_trend={}
        ...
        DIC_and_TIF(pixelsize=0.25).arr_to_tif(array_slope_mask,outdir+'GPCC_CV_trend.tif')
        DIC_and_TIF(pixelsize=0.25).arr_to_tif(array_p_value_mask,outdir+'GPCC_CV_p_value.tif')

        outf=outdir+'GPCC_CV_trend.npy'
...
class multi_regression_window():
    ...
    def run(self):
        ...
        self.window = 39-15
        outdir = result_root + rf'multi_regression_moving_window\\window15_relative_change\\'
        T.mk_dir(outdir, force=True)
    ...
    def convert_files_to_time_series(self, multi_regression_result_dir,y_var):
        ...
        variable_list= ['GLEAM_SMroot', 'Tempmean', 'VPD']
...
class multi_regression():
    def __init__(self):
        self.fdirX=result_root+rf'Detrend\detrend_relative_change\extend\\'
        self.fdirY=result_root+rf'Detrend\detrend_relative_change\extend\\'
...
class build_dataframe():
    ...
    def __init__(self):
        ...
        self.dff = self.this_class_arr + 'RF_df'
    ...
    def build_df(self, df):
        ...
        fdir =result_root+ rf'multi_regression_moving_window\window15_relative_change\npy_time_series\\'
        all_dic= {}
        for f in os.listdir(fdir):
    ...
    def add_trend_to_df(self,df):
        ...
        fdir=result_root+rf'trend_analysis\relative_change\OBS_LAI_extend\\'
...
class plot_dataframe():
    ...
    def plot_annual_zscore_based_region_climate_variables(self):   #based on semi-arid, arid and sub-humid
        ...
        greening_class_list=['sig_greening','non_sig_greening','sig_browning','non_sig_browning']

        # climate_variable_list = ['CRU','Precip','GLEAM_SMroot','GLEAM_SMsurf','VPD','tmin','tmax','Tempmean','GPCC','LAI4g']

        climate_variable_list = ['LAI4g', 'Precip', 'GLEAM_SMroot', 'GLEAM_SMsurf', 'VPD',
                                 'GPCC', ]

        color_list = ['blue', 'green', 'red', 'orange', 'black', 'yellow', 'purple', 'pink', 'grey', 'brown',
        ...
        for region in ['North_America']:
            ...
            for variable in climate_variable_list:
                ...
                for greening in greening_class_list:


                    df_greening = df_region[df_region['LAI4g_trend_class'] == greening]
                    print(len(df_greening))




                    column_name = variable
                    vals = df_greening[column_name].tolist()
                    ...
                    plt.plot(vals_mean, label=greening, color=color_list[flag])
...
class check_data():
    ...
    def plot_sptial(self):
        ...
        f =  rf'D:\Project3\Data\ERA5\ERA5_daily\dict\dry_spell\\'
        ...
        plt.imshow(arr,cmap='RdBu',interpolation='nearest',vmin=-20,vmax=30)
        plt.colorbar()
        plt.title(f)
        plt.show()
    def testrobinson(self):
        fdir=result_root+rf'trend_analysis\relative_change\TRENDY_LAI\\S2\\'
        ...
        # f = data_root + rf'split\NDVI4g\2001_2020.npy'
            Plot().plot_Robinson(fdir+f, vmin=-1,vmax=1,is_discrete=False,colormap_n=7,)
        #     cmap = T.gen_colors(7,'RdGy_r')
        #   cmap = T.cmap_blend(cmap)
        #     Plot().plot_Robinson(fdir + f, vmin=-0.01, vmax=0.01, is_discrete=True, colormap_n=7, cmap=cmap)
            # plt.title(f'{fname}')
            fname=f.split('.')[0]
            plt.title(f'{fname}')
