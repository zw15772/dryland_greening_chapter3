...
class Figure2_extract_LAI_percentile():
    def __init__(self):
        pass
    def run(self):
    ...
    def moving_window_percentile_anaysis(self): ## each window calculating the average
        ...
        T.mk_dir(outdir, force=True)
        percentiles = [1, 5, 10,20,30, 80,90, 95, 99]
...
class Figure_beta():
    ...
    def generate_bivarite_map(self):  ##
        ...
        import xymap
        tif_rainfall = result_root + rf'3mm\CRU_JRA\extract_rainfall_phenology_year\moving_window_average_anaysis_trend\growing_season\\trend\\\sum_rainfall_trend.tif'
        ...
        T.mk_dir(outdir, force=True)
        outtif = outdir + rf'\\wetting_beta_growing_season.tif'
        ...
        dics = {'wetting_drying': dic1,
                'beta': dic2}
        df = T.spatial_dics_to_df(dics)
        # print(df)
        df['wetting'] = df['wetting_drying'] > 0
        df['beta_increase'] = df['beta'] > 0

        print(df)
        label_list = []
        for i, row in df.iterrows():
            if row['wetting'] and row['beta_increase']:
                label_list.append(1)
            elif row['wetting'] and not row['beta_increase']:
                label_list.append(2)
            elif not row['wetting'] and row['beta_increase']:
                label_list.append(3)
            else:
                label_list.append(4)
    ...
    def generate_df(self):
        ...
        ftiff=result_root + rf'3mm\bivariate_analysis\\{variable}\\wetting_beta_growing_season.tif'
        ...
        f_CVLAItiff=result_root+rf'3mm\extract_composite_phenology_year\trend\\composite_LAI_relative_change_mean_trend.tif'
        array_CV_LAI,originX, originY, pixelWidth, pixelHeight=ToRaster().raster2array(f_CVLAItiff)
        array_CV_LAI[array_CV_LAI<-999]=np.nan
        dic_CV_LAI=DIC_and_TIF(pixelsize=0.5).spatial_arr_to_dic(array_CV_LAI)


        df=T.spatial_dics_to_df({'wetting_beta':dic_beta_CVrainfall,f'LAI_trend':dic_CV_LAI})

        T.save_df(df, result_root + rf'\3mm\bivariate_analysis\Dataframe\\three_dimension_wetting_beta_LAItrend.df')
        T.df_to_excel(df, result_root + rf'\3mm\bivariate_analysis\Dataframe\\three_dimension_wetting_beta_LAItrend.xlsx')
        # exit()
    ...
    def generate_three_dimension(self):
        variable='composite_LAI'
        dff=result_root + rf'\3mm\bivariate_analysis\Dataframe\\three_dimension_wetting_beta_LAItrend.df'
        df=T.load_df(dff)
        self.df_clean(df)
        # df=df[df['composite_LAI_CV_trend']>=0]


        for col in ['LAI_trend', 'wetting_beta']:
            df[col] = pd.to_numeric(df[col], errors='coerce')
            df.loc[df[col] <= -99, col] = np.nan  # 若没有哨兵可以删掉这一行

            # 2) 给 LAI_trend 打标签（NaN 安全）

        def trend_label(x):
            if pd.isna(x):
                return pd.NA
            return 'greening' if x >= 0 else 'browning'

        df['LAI_trend_label'] = df['LAI_trend'].apply(trend_label)

        # 3) 仅保留 wetting_beta ∈ {1,2,3,4}，其他置为 NA
        df['wetting_beta'] = df['wetting_beta'].astype('Int64')  # Int64 能保留 NA
        df.loc[~df['wetting_beta'].isin([1, 2, 3, 4]), 'wetting_beta'] = pd.NA

        # 4) 构造映射（MultiIndex + reindex，未匹配的自动给 NaN 而不是 KeyError）
        mapping = pd.Series({
            ('greening', 1): 1, ('greening', 2): 2, ('greening', 3): 3, ('greening', 4): 4,
            ('browning', 1): 5, ('browning', 2): 6, ('browning', 3): 7, ('browning', 4): 8,
        })

        mi = pd.MultiIndex.from_arrays([df['LAI_trend_label'], df['wetting_beta']])
        df['wetting_LAI_beta'] = mapping.reindex(mi).to_numpy()  # 没有匹配的得到 NaN

        # Display the result
        print(df)
        outdir = result_root + rf'\3mm\bivariate_analysis\\{variable}\\'
        T.mk_dir(outdir, force=True)
        outf = outdir + rf'LAI_wetting_beta_growing_season.tif'

        spatial_dic = T.df_to_spatial_dic(df, 'wetting_LAI_beta')
    ...
    def plot_figure2b(self):
        ...
        variable='composite'
        dff = rf'D:\Project3\Result\3mm\bivariate_analysis\Dataframe\three_dimension_wetting_beta_LAItrend.df'
        df = T.load_df(dff)
        df = self.df_clean(df)
        # df=df.dropna()
        ...
        df_greening = df[df[f'LAI_wetting_beta_growing_season'] < 5]
        count_green = len(df_greening)


        df_browning = df[df[f'LAI_wetting_beta_growing_season'] >= 5]
        ...
        for i in range(1, 9):
            ...
            if i < 5:
                df_i = df[df[f'LAI_wetting_beta_growing_season'] == i]
                count = len(df_i)
        ...
        # Aesthetics
        ax.set_yticks([0, 1])
        ax.set_yticklabels(['drying', 'wetting'])
        ax.set_xlabel('%')
        ax.set_xlim(0, 60)
    ...
    def plot_figure2b_test(self):
        ...
        variable='composite'
        dff = rf'D:\Project3\Result\3mm\bivariate_analysis\Dataframe\three_dimension_growing_season.df'
        df = T.load_df(dff)
        df = self.df_clean(df)
        df=df.dropna()
        # df_sig=df[df['detrended_SNU_LAI_CV_p_value']<0.05]
    ...
    def plot_figure2a_Robinson(self):
        ...
        for f in os.listdir(fdir_trend):
            ...
            fname = f.split('.')[0]
            if not 'LAI_wetting_beta_growing_season' in fname:
                continue
         ...
         # '#ff7f00', '#6a3d9a', '#b15928']
            ...
            # plt.title(f'{fname}')
            # plt.show()
            outf = outdir + 'LAI_wetting_beta_growing_season.pdf'
            plt.savefig(outf)
            plt.close()
            # exit()
...
class build_dataframe():
    ...
    def __init__(self):
        ...
        self.dff = self.this_class_arr + rf'three_dimension_wetting_beta_LAItrend.df'
    ...
    def add_8class_to_df(self, df):
        fdir=rf'D:\Project3\Result\3mm\bivariate_analysis\composite_LAI\\'
        for f in os.listdir(fdir):
            if not 'LAI_wetting_beta_growing_season' in f:
                continue
...
class partial_correlation():
    ...
    def statistic_corr_boxplot(self):
        ...
        for model in self.model_list:
            ...
            for new_var in new_variable_list:
                vals=df[new_var].to_list()
                vals=np.array(vals)
                vals[vals>99]=np.nan
                vals[vals<-99]=np.nan
                ...
                print(len(vals_positive)/len(vals))
                ## calculating average
                average=np.nanmean(vals)

                result_dic[f'{new_var}']=vals
    ...
    def plot_percentage(self):
        ...
        for var in var_list:
            ...
            # 如果你想算比例（分布归一化）
            hist_pos, bins = np.histogram(df_pos, bins=5, density=True)
            hist_neg, _ = np.histogram(df_neg, bins=5, density=True)
    ...
    def box_plot_significant_differences(self):
        ...
        # grouping columns
        corr_cols = {
            'Fq': 'composite_LAI_CV_intraannual_rainfall_ecosystem_year_zscore',
            'IAV': 'composite_LAI_detrended_sum_rainfall_ecosystem_year_CV_zscore',
        }
        ...
        # axes formatting
        ylims = {
            'sum_rainfall_trend': (-4, 7),
            'VPD_trend': (-0.0005, 0.0026),
            'sum_rainfall_mean': (-50, 801),
            'T_SAND': (0, 120),
            'Non_tree_vegetation_mean': (20, 101),

            'rooting_depth': (0,16),
        }
        yticks = {
            'sum_rainfall_trend': np.arange(-4, 7, 2),
            'VPD_trend': np.arange(-0.0005, 0.0026, 0.001),
            'sum_rainfall_mean': np.arange(-50, 801, 200),
            'T_SAND': np.arange(0, 120, 20),
            'Non_tree_vegetation_mean': np.arange(20, 101, 20),

            'rooting_depth': np.arange(0,16,5),
        }
        ylabels = {
            ...
            'T_SAND': 'Sand (%)',
            'Non tree vegetation_trend': 'Trends in short vegetation (%/yr)',
            'Non_tree_vegetation_mean': 'Mean short vegetation (%)',
            'rooting_depth': 'Rooting depth (cm)',
        }

        outdir = result_root + rf'\3mm\FIGURE\patial_corr\\boxplot\\'
        T.mk_dir(outdir, force=True)
        ...
        for var in var_list:
            ...
            for gi, patch in enumerate(bx['boxes']):
                ...
                bx['whiskers'][2 * gi + 1].set_color(EDGE[tag])
                bx['whiskers'][2 * gi].set_linewidth(1.2);
                bx['whiskers'][2 * gi + 1].set_linewidth(1.2)
                bx['caps'][2 * gi].set_color(EDGE[tag]);
                bx['caps'][2 * gi + 1].set_color(EDGE[tag])
            ...
            # plt.tight_layout()
            # plt.show()
            plt.savefig(outdir + f'{var}_boxplot.pdf')
            plt.close(fig)
    ...
    def sensitivity_vs_climate_factors(self):
        ...
        # 设置变量名
        target_var_list = [
            'composite_LAI_detrended_sum_rainfall_growing_season_CV_zscore',
            'composite_LAI_rainfall_frenquency_growing_season_zscore']
    ...
    def filter_percentile(self, var,tmp):
        dic_start = {}
        dic_end = {}
        ...
        dic_start[var] = percentile_values[0]
        dic_end[var] = percentile_values[1]
        df = tmp[(tmp[var] >= percentile_values[0]) & (tmp[var] <= percentile_values[1])]
    ...
    def percentage_pft(self):
        ...
        # 相关性（正/负）判断的列
        col_Fq = 'composite_LAI_CV_intraannual_rainfall_ecosystem_year_zscore'
        col_IAV = 'composite_LAI_detrended_sum_rainfall_ecosystem_year_CV_zscore'
...
class partial_correlation_TRENDY():
    ...
    def plot_partial_correlation(self,outdir):
        ...
        for var_i in var_list:
            ...
            DIC_and_TIF(pixelsize=0.5).arr_to_tif(arr, outdir + f'{var_i}.tif')
            std = np.nanstd(arr)
            mean = np.nanmean(arr)
            vmin = mean - std
            vmax = mean + std
    ...
    def plot_partial_correlation_p_value(self,outdir ):
        ...
        for var_i in var_list:
            ...
            DIC_and_TIF(pixelsize=0.5).arr_to_tif(arr, outdir + f'{var_i}_p_value.tif')
            std = np.nanstd(arr)
            mean = np.nanmean(arr)
            vmin = mean - std
            vmax = mean + std
    ...
    def TRENDY_obs_heatmap(self):
        dff = result_root + rf'3mm\Multiregression\partial_correlation\Obs\result\Dataframe\\Dataframe.df'
        df = T.load_df(dff)
        df = self.df_clean(df)


        model_list = ['composite_LAI_median',  'LAI4g','GLOBMAP_LAI','SNU_LAI',
                          'TRENDY_ensemble_median', 'CABLE-POP_S2_lai', 'CLASSIC_S2_lai',
                          'CLM5', 'DLEM_S2_lai', 'IBIS_S2_lai', 'ISAM_S2_lai',
                          'ISBA-CTRIP_S2_lai', 'JSBACH_S2_lai',
                          'JULES_S2_lai', 'LPJ-GUESS_S2_lai', 'LPX-Bern_S2_lai',
        ...
        result_dic={}
        heatmap_data = pd.DataFrame(index=variable_list, columns=model_list)

        # 填充数据（使用中位数或均值）
        for variable in variable_list:
            for model in model_list:
                df = df.dropna(subset=f'{model}_{variable}')
                col = f'{model}_{variable}'
                vals = df[col].values
                vals = np.where((vals > 100) | (vals < -100), np.nan, vals)
                median_val = np.nanmean(vals)
                heatmap_data.loc[variable, model] = median_val

        # 转换为 float 类型（必要时）
        heatmap_data = heatmap_data.astype(float)

        dic_label_name = {'composite_LAI_median': 'Composite',
                          'LAI4g': 'GIMMS4g',
                          'SNU_LAI': 'SNU',
                          'GLOBMAP_LAI': 'GLOBMAP',
        ...
        dic_variable_name={'sensitivity_zscore':'$\gamma$',
                           'CV_intraannual_rainfall_ecosystem_year_zscore':'CVintra-rainfall',

                           'detrended_sum_rainfall_ecosystem_year_CV_zscore':'CVinter-rainfall'}

        # 绘图
        plt.figure(figsize=(9, 2.5))  # 宽度14，高度3让每行比较清晰
        # sns.heatmap(heatmap_data, annot=True, cmap='coolwarm_r', cbar_kws={'label': 'Median Coefficient'},
        #             vmin=-0.5, vmax=0.5,
        #
        #             linewidths=0.5)

        sns.heatmap(heatmap_data,cmap='coolwarm_r', cbar_kws={'label': 'Coefficient'},
                    vmin=-0.3, vmax=0.3,

                    linewidths=0.5)


        ## use dic_name to replace model name
        ax = plt.gca()
        ax.set_xticklabels(dic_label_name.values(), rotation=90, fontsize=10, font='Arial')
        ax.set_yticklabels(dic_variable_name.values(), rotation=0, fontsize=10, font='Arial')
        outdir=result_root + rf'\3mm\FIGURE\Figure3_attribution\\'
        T.mkdir(outdir, force=True)
        plt.tight_layout()

        plt.savefig(outdir + rf'heatmap.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    def TRENDY_obs_boxplots(self): ## obs and TRENDY boxplots comparison
        dff = result_root + rf'3mm\Multiregression\partial_correlation\Obs\obs_climate\Dataframe\\partial_correlation.df'
        df = T.load_df(dff)
        df = self.df_clean(df)
        df=df.dropna(axis=0, how='any')

        model_list = ['composite_LAI', 'SNU_LAI', 'GLOBMAP_LAI', 'LAI4g',
        ...
        variable_list=['sensitivity_zscore','rainfall_frenquency_zscore',
        ...
        for variable in variable_list:
            result_dic = {}
            for model in model_list:
                col = f'{model}_{variable}_sig'
                vals = df[col].values
                vals = np.where((vals > 100) | (vals < -100), np.nan, vals)

                result_dic[model]=vals

            df_new = pd.DataFrame(result_dic).T  # 行是model，列是样本


            df_new.index.name = "Model"
            ...
            ax.set_title(dic_variable_name.get(variable, variable), fontsize=12, fontweight="bold")
            ax.set_ylabel(dic_variable_name.get(variable, variable), fontsize=11)
